import type { RoshRisks, RoshRisksEnvelope, TaskListErrors } from '@approved-premises/ui'
...
export default class Summary implements TaskListPage {
  ...
  constructor(
    ...
    const roshQuestions = getQuestions(this.personName)['risks-of-serious-harm-to-others']
    this.questions = {
    ...
    if (this.riskData) {
      ...
      if (isOasysData) {
        ...
        response = {
          ...
          ...Object.fromEntries(
            Object.entries(commonData).map(([label, key]) => [label, (oasysData.value as Record<string, string>)[key]]),
          ),
        }
      } else {
        ...
        response = {
          ...
          ...Object.fromEntries(
            Object.entries(commonData).map(([label, key]) => [
              label,
              (manualData as unknown as Record<string, string>)[key],
            ]),
          ),
        }
      }
  ...
  private getRiskDataSource(roshData: unknown): SummaryData | null {
    const riskData = roshData as Record<string, SummaryData>
