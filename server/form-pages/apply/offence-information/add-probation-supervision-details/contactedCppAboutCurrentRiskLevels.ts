...
import { dateAndTimeInputsAreValidDates, dateIsTodayOrInThePast } from '../../../../utils/dateUtils'
...
export default class ContactedCppAboutCurrentRiskLevels implements TaskListPage {
  ...
  errors() {
    ...
    if (!this.body.hasContactedCppAboutCurrentRiskLevels) {
      ...
      !this.body['contactDate-day'] &&
      !this.body['contactDate-month'] &&
      !this.body['contactDate-year']
    ) {
      errors.contactDate = 'Enter the date the CPP was contacted'
    } else if (
      this.body.hasContactedCppAboutCurrentRiskLevels === 'yes' &&
      !dateAndTimeInputsAreValidDates(this.body, 'contactDate')
    ) {
      errors.contactDate = 'Date of contact must include a day, month and year'
    } else if (
      this.body.hasContactedCppAboutCurrentRiskLevels === 'yes' &&
      !dateIsTodayOrInThePast(this.body, 'contactDate')
    ) {
