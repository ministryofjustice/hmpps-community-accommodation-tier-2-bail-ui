import { SubmitCas2Application, UpdateCas2Application, ApplicationOrigin } from '@approved-premises/api'
...
describeClient('ApplicationClient', provider => {
  ...
  describe('getAllByOrigin', () => {
    it('should get all applications for a given applicationOrigin', async () => {
      ...
      const result = await applicationClient.getAllByOrigin('prisonBail', undefined, 1)
    ...
    it('should get applications for a specific CRN when a user searches for a CRN', async () => {
      const crn = 'A123456'

      const applications = [
        applicationSummaryFactory.build({ crn }),
        applicationSummaryFactory.build({ crn }),
        applicationSummaryFactory.build({ crn }),
      ]
      ...
      const result = await applicationClient.getAllByOrigin('prisonBail', crn, 1)
