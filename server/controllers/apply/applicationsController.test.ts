...
describe('applicationsController', () => {
  ...
  describe('selectApplicationOrigin', () => {
    beforeEach(() => {
      response = createMock<Response>({
        locals: { user: { userRoles: ['PROBATION'] } },
      })
    })
    ...
    it('redirects to the unauthorised court bail page when the application origin is courtBail and the user does not have the PROBATION role', async () => {
      request.body = {
        applicationOrigin: 'courtBail' as ApplicationOrigin,
      }

      response = createMock<Response>({
        locals: { user: { userRoles: ['PRISON'] } },
      })
